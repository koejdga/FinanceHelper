...
export default function ResetPassword({navigation}) {
    ...
    const checkEmail = () => {
        ...
        if(!emailCorrectness.isValid){
            ...
            sendPasswordResetEmail(appAuth, email).then(() => {
                let message = "Email for password change was sent to this address";
                Alert.alert("", message, [{text: "OK", onPress: navigation.navigate("Login")}]);
            }).catch(() => {
    ...
    return (
        <GestureHandlerRootView>
            <ScrollView>
                <View style={[formContainerStyles.alignCenter,
                    {width: "90%", marginTop: "45%"}]}>
                    <Text style={[formContainerStyles.alignCenter, formContainerStyles.title]}>Reset password</Text>
                    <FormTextInput
                    ...
                    <BorderlessButton
                        ...
                        <Text style={formStyles.buttonText}>Send me an email to reset password</Text>
                    </BorderlessButton>
                    <Pressable onPress={() => navigation.navigate("Login")}>
                        <Text style={[formContainerStyles.accentBoldText, formContainerStyles.alignCenter]}>Back to login</Text>
                    </Pressable>
                </View>
            </ScrollView>
        </GestureHandlerRootView>
